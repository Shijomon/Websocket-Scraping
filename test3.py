...
async def connect_and_send_message():
    ...
    proxy_url = 'http://'  # Adjust this line
    ...
    response = requests.get(
        ...
        headers=headers,proxies={'https':proxy_url}
    )
    print(response)
    ...
    async with ClientSession() as session:
        async with session.ws_connect(
            ...
            await websocket.send_str(message1)
            #logging.debug("Sent CONNECT frame: %s", message1)
            message2 = f"""SUBSCRIBE\nid:sub-0\ndestination:/topic/vendorRates.{searchid}\n\n\x00"""
            await websocket.send_str(message2)
            #logging.debug("Sent SUBSCRIBE frame: %s", message2)
            message3 = f"""SUBSCRIBE\nid:sub-1\ndestination:/topic/searchCompleted.{searchid}\n\n\x00"""
            await websocket.send_str(message3)
            #logging.debug("Sent SUBSCRIBE frame: %s", message3)
            message4 = f'SEND\ndestination:/app/search.{searchid}\ncontent-length:31\n\nstart async search for{searchid}\x00'
            await websocket.send_str(message4)
            #logging.debug("Sent SEND frame: %s", message4)
            try:
                while True:
                    raw_response = await websocket.receive()
                    # #logging.debug("Raw received response: %s", raw_response.data)
                    # if "CONNECTED" in raw_response.data:
                    #     logging.info("Successfully connected to the server")
                    if "MESSAGE" in raw_response.data:
                        logging.info("Received a message: %s", raw_response.data)
                    # elif "ERROR" in raw_response.data:
                    #     logging.error("Error frame received: %s", raw_response.data)
                    if "searchCompleted" in raw_response.data:
